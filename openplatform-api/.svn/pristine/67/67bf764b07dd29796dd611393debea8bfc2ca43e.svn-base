...
/**
 * 利用twitter的snowflake算法实现ID分配 其原理结构如下，我分别用一个0表示一位，用—分割开部分的作用： 0---0000000000
 ...
 * <p/>
 * 针对项目改进如下： 0 ---0000000000 0000000000 0000000000 0000000000 0 ---00 ---000
 * ---00000 ---000000000000
...
public final class GenIDUtil {
    ...
    /**
     * 基准时间戳 2018-12-01 00:00:00:000
     */
    private static final long refTimestamp = 1451577600000L;
    ...
    private static void genSingleID() {
        ...
        log.debug("genSingleID result : {} ", id);
    }
    ...
    private static void genBatchId() {
        class Test implements Runnable {
            ...
            public void run() {
                ...
                System.out.println("finish : " + (end - begin));
            }
        }
    ...
    public synchronized long nextId() {
        ...
        if (this.lastTimestamp == timestamp) {
            this.sequence = (this.sequence + 1) & GenIDUtil.sequenceMask;
            if (this.sequence == 0) {
        ...
        long nextId = ((timestamp - refTimestamp << timestampLeftShift))
                | ((this.dataCenterId & dataCenterMask) << GenIDUtil.dataCenterLeftShift)
                | ((this.machineId & machineMask) << GenIDUtil.machineLeftShift)
                | ((curThreadId & threadMask) << GenIDUtil.threadLeftShift) | (this.sequence);
